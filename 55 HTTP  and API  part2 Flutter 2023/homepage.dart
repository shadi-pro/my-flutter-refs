...
class _HomepageState extends State<Homepage> {
  ...
  // 1- define a list of the obtained data to recieve inside it  :
  List data = [];

  // 2-  define a variable of loading status [with false value by default] :
  bool loading = false;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Package part 2'), actions: []),
        ...
        children: [
          Container(
            ...
            child: MaterialButton(
              ...
              // a- using the {get} method of [http package]  inside the (onPressed) of button to get data form the assiend url +  using the [async-await]  :
              onPressed: () async {
                // 0- setting the loading data with true before displaying  data  below :
                loading = true;
                setState(() {}); // for dynamic refresh  displaying
                ...
                // 2- define  [responsebody]  variable assigned with  converted obtained data value form [json string] into  list of maps by using the [jsonDecode] method  :
                var bodyresponse = jsonDecode(response.body);

                // 3- adding the obtained data converted to list  [bodyresponse] inside the defined list as an addition to the list :
                data.addAll(bodyresponse);

                // 4- reset the loading variable by false again  :
                loading = false;

                // 5- using the {setState} to dynamic display above data into the UI  [ for dynamic refresh  displaying ] :
                setState(() {});
              },
          ...
          //  b- Conditional displaying of loading indicator {CircularProgressIndicator}  - incase of  the loading is true -   :
          if (loading) Center(child: CircularProgressIndicator()),

          // c- Generate a list and add the obtained data inside :
          ...List.generate(
            data.length,
            (index) => Card(
              child: ListTile(
                title: Text(
                  "title of ${data[index]['userId']} :  ${data[index]['title']}",
                ),
