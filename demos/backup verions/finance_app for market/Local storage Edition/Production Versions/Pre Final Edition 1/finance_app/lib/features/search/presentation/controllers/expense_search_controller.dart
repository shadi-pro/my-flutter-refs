...
class ExpenseSearchController extends GetxController {
  static const String _tag = 'ExpenseSearchController';

  final RxString searchQuery = ''.obs;
  final RxString selectedCategory = 'all'.tr.obs;
  final RxString selectedType = 'all'.tr.obs;
  final RxDouble minAmount = 0.0.obs;
  final RxDouble maxAmount = 1000000.0.obs;
  ...
  List<String> getAvailableCategories() {
    final Set<String> categories = <String>{'all'.tr};
    final ExpenseController expenseController = Get.find<ExpenseController>();

    for (final ExpenseEntity expense in expenseController.expenses) {
  ...
  List<Map<String, dynamic>> get filteredExpenses {
    try {
      final String currentHash = _getFilterHash();
      ...
      List<ExpenseEntity> expenseEntities =
          List<ExpenseEntity>.from(expenseController.expenses);

      // Search by query
      if (searchQuery.value.isNotEmpty) {
        expenseEntities = expenseEntities.where((ExpenseEntity expense) {
      ...
      final List<Map<String, dynamic>> result = expenseEntities
          .map((ExpenseEntity expense) => expense.toMap())
          .toList();
      ...
      ErrorHandler.showError(
        'filter_error_message'.trParams(<String, String>{'e': e.toString()}),
      );
  ...
  List<ExpenseEntity> get filteredExpenseEntities {
    try {
      ...
      if (searchQuery.value.isNotEmpty) {
        expenses = expenses.where((ExpenseEntity expense) {
          final String categoryDisplay =
              _getDisplayCategory(expense.category.translationKey);
          return categoryDisplay
                  .toLowerCase()
      ...
      expenses
          .sort((ExpenseEntity a, ExpenseEntity b) => b.date.compareTo(a.date));
  ...
  List<ExpenseEntity> quickSearch(String query) {
    final ExpenseController expenseController = Get.find<ExpenseController>();
    return expenseController.expenses.where((ExpenseEntity expense) {
      final String categoryDisplay =
          _getDisplayCategory(expense.category.translationKey);
