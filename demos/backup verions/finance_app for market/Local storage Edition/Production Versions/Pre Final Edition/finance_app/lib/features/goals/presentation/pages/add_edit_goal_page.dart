...
class _AddEditGoalPageState extends State<AddEditGoalPage> {
  ...
  Future<void> _selectDate(BuildContext context, bool isStartDate) async {
    ...
    if (picked != null) {
      setState(() {
        if (isStartDate) {
          _startDate = picked;
          // if the start date is after target date then update  target date
          if (_targetDate.isBefore(_startDate)) {
            _targetDate = _startDate.add(const Duration(days: 30));
          }
        } else {
          _targetDate = picked;
          // if the target is before start date then update start date
          if (_targetDate.isBefore(_startDate)) {
  ...
  void _saveGoal() {
    ...
    try {
      ...
      if (targetAmount <= 0) {
        throw Exception('target_amount_must_be_greater_than_zero'.tr);
      }

      if (currentAmount < 0) {
        throw Exception('current_amount_cannot_be_negative'.tr);
      }

      if (currentAmount > targetAmount) {
        throw Exception('current_cannot_exceed_target'.tr);
      }

      if (_targetDate.isBefore(_startDate)) {
        throw Exception('target_date_cannot_be_before_start'.tr);
      }
      ...
      ErrorHandler.showError(
        'error_saving_goal'.trParams({'error': e.toString()}),
      );
    }
  }
