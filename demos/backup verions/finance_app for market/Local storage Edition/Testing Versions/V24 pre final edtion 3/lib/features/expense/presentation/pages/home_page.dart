  ...
  Widget _buildExpenseItemWithSwipe(dynamic expense, int index) {
    return Dismissible(
      ...
      child: _buildExpenseItem(
          expense, index), // Calling the basic build function after swipe removing
    );
  }
    ...
    return Card(
      ...
      child: Directionality(
        ...
        child: ListTile(
          ...
          title: Text(
            _getTranslatedCategory(expense['category']), // ✅ FIXED: Translated category
        ...
        child: Column(
          ...
          children: [
            ...
            _buildDetailRow('${'category'.tr}:',
                _getTranslatedCategory(expense['category'])), // ✅ FIXED
            if (expense['description'].toString().isNotEmpty)
              _buildDetailRow(
            ...
            _buildDetailRow('${'payment_method'.tr}:',
                _getTranslatedPaymentMethod(expense['paymentMethod'] ?? 'cash')), // ✅ FIXED
            const SizedBox(height: 20),
            Row(
              children: [
                Expanded(
  ...
  void _duplicateExpense(dynamic expense) {
    try {
      ...
      if (_isArabicText(expense['paymentMethod'] ?? 'cash')) {
        paymentKey = _convertArabicToPaymentKey(expense['paymentMethod'] ?? 'cash');
      } else {
      ...
      final newExpense = ExpenseEntity(
        ...
        paymentMethod: PaymentMethod.fromKey(paymentKey), // ✅ Use fromKey
      );
