...
class BudgetController extends GetxController {
  ...
  Future<void> loadBudgetData() async {
    try {
      monthlyBudget.value = _storage.read('monthlyBudget') ?? 0.0;

      final storedBudgets = _storage.read('categoryBudgets') ?? {};
      categoryBudgets.value = Map<String, double>.from(storedBudgets);
  ...
  Future<void> setMonthlyBudget(double amount) async {
    ...
    await _storage.write('monthlyBudget', amount);
    update();
  }
  ...
  Future<void> _saveCategoryBudgets() async {
    await _storage.write('categoryBudgets', categoryBudgets);
  }

  // الحصول على إجمالي ميزانية الفئات
  ...
  Future<void> resetBudget() async {
    ...
    _initializeDefaultBudgets();
    await _storage.remove('monthlyBudget');
    await _storage.remove('categoryBudgets');
    update();
  }
}
