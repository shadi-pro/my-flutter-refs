...
class ExpenseEntity {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'amount': amount,
      'category':
          category.translationKey, // ✅ Store TRANSLATION KEY, not Arabic text
      'description':
          description.trim(), // ✅ Simplified - no UTF-8 encode/decode needed
      'date': date.toIso8601String(),
      'isIncome': isIncome,
      'receiptImage': receiptImage,
      'paymentMethod': paymentMethod.translationKey, // ✅ Store TRANSLATION KEY
    };
  }

  factory ExpenseEntity.fromMap(Map<String, dynamic> map) {
    try {
      // ✅ Use fromKey() instead of fromString()
      final categoryKey = map['category']?.toString() ?? 'others';
      final paymentMethodKey = map['paymentMethod']?.toString() ?? 'cash';
      ...
      return ExpenseEntity(
        ...
        category: ExpenseCategory.fromKey(categoryKey), // ✅ Use fromKey
        description: descriptionStr, // ✅ No UTF-8 encoding needed
        date: _parseDate(map['date']?.toString()),
        isIncome: _parseBoolean(map['isIncome']),
        receiptImage: map['receiptImage']?.toString(),
        paymentMethod: PaymentMethod.fromKey(paymentMethodKey), // ✅ Use fromKey
      );
    } catch (e, stackTrace) {
