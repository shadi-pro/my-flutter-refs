...
import 'package:finance_app/core/utils/secure_storage.dart';
...
class ExpenseController extends GetxController {
  ...
  Future<void> _initializeData() async {
    ...
    try {
      await SecureStorage.migrateOldData();
      await loadExpenses();
  ...
  Future<void> loadExpenses() async {
    try {
      final storedData = SecureStorage.readEncrypted('expenses');
  ...
  Future<void> addExpense(ExpenseEntity expense) async {
    try {
      ...
      if (expense.description.trim().isEmpty) {
        throw Exception('الوصف لا يمكن أن يكون فارغاً');
      }
  ...
  Future<void> clearAllData() async {
    try {
      ...
      expenses.clear();
      await SecureStorage.remove('expenses');
      calculateRealTotals();
  ...
  Future<void> _saveToStorage() async {
    try {
      ...
      await SecureStorage.writeEncrypted('expenses', data);
    } catch (e) {
